...
import (
	...
	apisCluster "github.com/exoscale/provider-exoscale/apis/cluster"
	apisNamespaced "github.com/exoscale/provider-exoscale/apis/namespaced"
	"github.com/exoscale/provider-exoscale/config"
	"github.com/exoscale/provider-exoscale/internal/clients"
	controllerCluster "github.com/exoscale/provider-exoscale/internal/controller/cluster"
	controllerNamespaced "github.com/exoscale/provider-exoscale/internal/controller/namespaced"
	"github.com/exoscale/provider-exoscale/internal/features"
	"github.com/exoscale/provider-exoscale/internal/version"
)
...
func main() {
	...
	log := logging.NewLogrLogger(zl.WithName("provider-exoscale"))
	if *debug {
	...
	mgr, err := ctrl.NewManager(cfg, ctrl.Options{
		...
		LeaderElectionID: "crossplane-leader-election-provider-exoscale",
		Cache: cache.Options{
	...
	kingpin.FatalIfError(apisCluster.AddToScheme(mgr.GetScheme()), "Cannot add cluster-scoped Exoscale APIs to scheme")
	kingpin.FatalIfError(apisNamespaced.AddToScheme(mgr.GetScheme()), "Cannot add namespaced Exoscale APIs to scheme")
	...
	if canSafeStart {
		...
		kingpin.FatalIfError(controllerCluster.SetupGated(mgr, clusterOpts), "Cannot setup cluster-scoped Exoscale controllers")
		kingpin.FatalIfError(controllerNamespaced.SetupGated(mgr, namespacedOpts), "Cannot setup namespaced Exoscale controllers")
	} else {
		log.Info("Provider has missing RBAC permissions for watching CRDs, controller SafeStart capability will be disabled")
		kingpin.FatalIfError(controllerCluster.Setup(mgr, clusterOpts), "Cannot setup cluster-scoped Exoscale controllers")
		kingpin.FatalIfError(controllerNamespaced.Setup(mgr, namespacedOpts), "Cannot setup namespaced Exoscale controllers")
	}
